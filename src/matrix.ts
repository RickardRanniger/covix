...
export class Matrix {
  ...
  isCollision(block: Block, destX: number, destY: number, destO: Orientation) {
    for (var y = 0; y < block.blockType[destO].length; y++) {
      for (var x = 0; x < block.blockType[destO][y].length; x++) {
        console.log(
  ...
  addBlock = (block: Block, tileType: TileType) => {
    for (var y = 0; y < block.blockType[block.rotation].length; y++) {
      for (var x = 0; x < block.blockType[block.rotation][y].length; x++) {
